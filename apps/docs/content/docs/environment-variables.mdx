...
| `AWS_ENDPOINT_URL` | empty | Custom S3-compatible endpoint (for example [MinIO](https://min.io/) or Tencent COS). Setting this switches to path-style URLs by default — see `S3_FORCE_PATH_STYLE` |
| `S3_FORCE_PATH_STYLE` | `true` when `AWS_ENDPOINT_URL` is set | Set to `false` for S3-compatible backends that require virtual-hosted-style requests instead of path-style, e.g. **Tencent COS**, which rejects path-style requests with `PathStyleDomainForbidden`. Has no effect unless `AWS_ENDPOINT_URL` is also set |
| `S3_KEY_PREFIX` | empty | Optional object key prefix (for example `multica-prod`), useful when sharing one bucket/CDN domain with other applications. Surrounding whitespace and leading/trailing slashes are trimmed. Leave empty to keep the existing unprefixed keys. **Set it before the first upload** — changing it later does not migrate already-uploaded objects, so their downloads start 404ing and deletes silently no-op until you copy them under the new prefix yourself |
...
**Stored object URLs** are constructed in this order of priority. When `S3_KEY_PREFIX` is set, `<key>` below is `<S3_KEY_PREFIX>/<key>`:

1. `https://<CLOUDFRONT_DOMAIN>/<key>` if `CLOUDFRONT_DOMAIN` is set.
2. If `AWS_ENDPOINT_URL` is set: `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>` (path-style, the default), or `<scheme>://<S3_BUCKET>.<endpoint-host>/<key>` (virtual-hosted-style) when `S3_FORCE_PATH_STYLE=false`.
...
| `MULTICA_OPENCLAW_CLI_TIMEOUT_SECONDS` | `5` | Timeout for each `openclaw config ...` call the daemon makes while preparing an OpenClaw task's environment. Raise this if task prep fails with `signal: killed` on slower machines or under concurrent task dispatch — a cold `openclaw` CLI invocation has been measured at 2.4-4.2s alone and 7+s when several tasks start at once. Unset, empty, non-numeric, or non-positive values fall back to the default |
