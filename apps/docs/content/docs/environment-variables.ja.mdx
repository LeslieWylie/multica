...
| `AWS_ENDPOINT_URL` | 空 | カスタムの S3 互換エンドポイント（例: [MinIO](https://min.io/) や Tencent COS）。これを設定するとデフォルトで path-style URL に切り替わります — `S3_FORCE_PATH_STYLE` を参照 |
| `S3_FORCE_PATH_STYLE` | `AWS_ENDPOINT_URL` 設定時はデフォルト `true` | 一部の S3 互換バックエンドは path-style ではなく virtual-hosted-style のリクエストを要求します。例えば **Tencent COS** は path-style リクエストを `PathStyleDomainForbidden` で拒否します — その場合は `false` に設定してください。`AWS_ENDPOINT_URL` が未設定の場合は効果がありません |
| `S3_KEY_PREFIX` | 空 | オプションのオブジェクト key プレフィックス（例: `multica-prod`）。1 つのバケット/CDN ドメインを他のアプリケーションと共有する場合に便利です。前後の空白と先頭・末尾のスラッシュは自動的に取り除かれます。空のままにすると既存のプレフィックスなしの key が維持されます。**必ず最初のアップロード前に設定してください** — 後から変更しても既存のオブジェクトは移行されないため、既存の添付ファイルはダウンロードが 404 になり、削除は静かに失敗します。手動で新しいプレフィックス配下にコピーしない限り復旧しません |
...
**保存されるオブジェクト URL** は次の優先順位で構築されます。`S3_KEY_PREFIX` が設定されている場合、以下の `<key>` はすべて `<S3_KEY_PREFIX>/<key>` になります。

1. `CLOUDFRONT_DOMAIN` が設定されている場合は `https://<CLOUDFRONT_DOMAIN>/<key>`。
2. `AWS_ENDPOINT_URL` が設定されている場合、デフォルトは `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>`（path-style）。`S3_FORCE_PATH_STYLE=false` の場合は `<scheme>://<S3_BUCKET>.<endpoint-host>/<key>`（virtual-hosted-style）になります。
...
| `MULTICA_OPENCLAW_CLI_TIMEOUT_SECONDS` | `5` | デーモンが OpenClaw タスクの環境を準備する際に行う各 `openclaw config ...` 呼び出しのタイムアウト。低速なマシンや複数タスクの同時実行時に `signal: killed` でタスク準備が失敗する場合は値を上げてください——`openclaw` CLI のコールドスタートは単体で 2.4-4.2 秒、複数タスク同時起動時は 7 秒以上かかることが確認されています。未設定・空・数値以外・0以下の値はデフォルトにフォールバックします |
