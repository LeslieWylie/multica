...
| `AWS_ENDPOINT_URL` | 비어 있음 | 사용자 정의 S3 호환 엔드포인트 (예: [MinIO](https://min.io/) 또는 Tencent COS). 이를 설정하면 기본적으로 path-style URL로 전환됩니다 — `S3_FORCE_PATH_STYLE` 참고 |
| `S3_FORCE_PATH_STYLE` | `AWS_ENDPOINT_URL` 설정 시 기본값 `true` | 일부 S3 호환 백엔드는 path-style 대신 virtual-hosted-style 요청을 요구합니다. 예를 들어 **Tencent COS**는 path-style 요청을 `PathStyleDomainForbidden`으로 거부합니다 — 이 경우 `false`로 설정하세요. `AWS_ENDPOINT_URL`이 설정되지 않으면 효과가 없습니다 |
| `S3_KEY_PREFIX` | 비어 있음 | 선택적 객체 key 접두사 (예: `multica-prod`). 하나의 버킷/CDN 도메인을 다른 애플리케이션과 공유할 때 유용합니다. 앞뒤 공백과 슬래시는 자동으로 제거됩니다. 비워두면 기존의 접두사 없는 key가 그대로 유지됩니다. **첫 업로드 전에 반드시 설정하세요** — 나중에 변경하면 기존에 업로드된 객체는 마이그레이션되지 않아 다운로드는 404가 되고 삭제는 조용히 무시됩니다. 새 접두사 아래로 직접 복사하지 않는 한 복구되지 않습니다 |
...
**저장된 객체 URL**은 다음 우선순위 순서로 구성됩니다. `S3_KEY_PREFIX`가 설정된 경우 아래의 `<key>`는 모두 `<S3_KEY_PREFIX>/<key>`가 됩니다:

1. `CLOUDFRONT_DOMAIN`이 설정된 경우 `https://<CLOUDFRONT_DOMAIN>/<key>`.
2. `AWS_ENDPOINT_URL`이 설정된 경우, 기본값은 `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>` (path-style). `S3_FORCE_PATH_STYLE=false`이면 `<scheme>://<S3_BUCKET>.<endpoint-host>/<key>` (virtual-hosted-style)가 됩니다.
...
| `MULTICA_OPENCLAW_CLI_TIMEOUT_SECONDS` | `5` | 데몬이 OpenClaw 작업 환경을 준비할 때 실행하는 각 `openclaw config ...` 호출의 제한 시간. 느린 머신이나 여러 작업이 동시에 시작될 때 작업 준비가 `signal: killed`로 실패하면 이 값을 늘리세요 — 콜드 스타트 상태의 `openclaw` CLI 호출은 단독으로 2.4-4.2초, 동시 여러 작업 시작 시 7초 이상이 측정되었습니다. 설정하지 않거나 비어 있거나 숫자가 아니거나 양수가 아닌 값은 기본값으로 대체됩니다 |
