...
| `AWS_ENDPOINT_URL` | 空 | 自定义 S3 兼容端点（例如 [MinIO](https://min.io/) 或腾讯云 COS）。设了默认会切到 path-style URL——见 `S3_FORCE_PATH_STYLE` |
| `S3_FORCE_PATH_STYLE` | 设了 `AWS_ENDPOINT_URL` 时默认 `true` | 部分 S3 兼容后端要求走 virtual-hosted-style 而不是 path-style，比如**腾讯云 COS**，用 path-style 会直接报 `PathStyleDomainForbidden`——这种情况下设为 `false`。不设 `AWS_ENDPOINT_URL` 时这个变量不生效 |
| `S3_KEY_PREFIX` | 空 | 可选的对象 key 前缀（例如 `multica-prod`），用于多个应用共用同一个 bucket/CDN 域名时做目录隔离。首尾空白和斜杠都会被自动去掉。留空则保持现有不带前缀的 key。**请在第一次上传前就设置好**——上线后再修改不会迁移已有对象，会导致已有附件下载报 404、删除静默失效，除非手动把它们复制到新前缀目录下 |
...
**对象存储 URL** 按优先级拼装；设了 `S3_KEY_PREFIX` 时，下面的 `<key>` 均为 `<S3_KEY_PREFIX>/<key>`：

1. 设了 `CLOUDFRONT_DOMAIN` → `https://<CLOUDFRONT_DOMAIN>/<key>`
2. 设了 `AWS_ENDPOINT_URL` → 默认 `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>`（path-style）；`S3_FORCE_PATH_STYLE=false` 时改为 `<scheme>://<S3_BUCKET>.<endpoint-host>/<key>`（virtual-hosted-style）
...
| `MULTICA_OPENCLAW_CLI_TIMEOUT_SECONDS` | `5` | 守护进程准备 OpenClaw 任务环境时，每次调用 `openclaw config ...` 的超时时间。如果任务准备在较慢的机器上或并发派发任务时报 `signal: killed`，可以调大这个值——实测单次冷启动 `openclaw` CLI 需要 2.4-4.2 秒，多个任务并发启动时超过 7 秒。未设置、空值、非数字或非正数都会回退到默认值 |
