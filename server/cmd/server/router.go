	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				// Octo IM mutations stay admin-only. Unlike Lark (MUL-4213,
				// where begin/revoke self-check via canManageAgent inside the
				// handler), the Octo create/revoke handlers rely on this
				// router-level role gate for authorization.
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceRoleFromURL(queries, "id", "owner", "admin"))
